...
def assembly_result_from_dict(data: Mapping[str, Any]) -> AssemblyResult:
    ...
    return AssemblyResult(
        ordered_elements=[
            _assembly_element_from_dict(item)
            for item in _coerce_mapping_list(payload.get("ordered_elements"))
        ],
        block_relations=[
            _block_relation_from_dict(item)
            for item in _coerce_mapping_list(payload.get("block_relations"))
        ],
        document=_document_from_dict(_coerce_mapping(payload.get("document"))),
        page_stats=[
            _page_stats_from_dict(item)
            for item in _coerce_mapping_list(payload.get("page_stats"))
        ],
        warnings=[
            _assembly_warning_from_dict(item)
            for item in _coerce_mapping_list(payload.get("warnings"))
        ],
        metadata=_meta_from_dict(_coerce_mapping(payload.get("metadata"))),
        raw=payload.get("raw"),
    )


def _document_from_dict(data: Mapping[str, Any]) -> AssembledDocument:
    ...
    return AssembledDocument(
        ...
        children=[
            _assembled_node_from_dict(item)
            for item in _coerce_mapping_list(payload.get("children"))
        ],
        sections=[
            _section_node_from_dict(item)
...
def _assembled_node_from_dict(data: Mapping[str, Any]) -> Any:
    """직렬화된 document child를 node type 기준으로 복원한다."""
    payload = dict(data)
    node_type = payload.get("type")
    if node_type == "section":
        return _section_node_from_dict(payload)
    if node_type == "paragraph_group":
        return _paragraph_group_from_dict(payload)
    if node_type == "list_group":
        return _list_group_from_dict(payload)
    if node_type == "table_ref":
        return _table_ref_from_dict(payload)
    if node_type == "figure_ref":
        return _figure_ref_from_dict(payload)
    if node_type == "note_ref":
        return _note_ref_from_dict(payload)
    raise ValueError(f"Unknown assembled node type: {node_type!r}")


def _section_node_from_dict(data: Mapping[str, Any]) -> SectionNode:
    payload = dict(data)
    return SectionNode(
        type="section",
...
def _paragraph_group_from_dict(data: Mapping[str, Any]) -> ParagraphGroup:
    payload = dict(data)
...
def _list_group_from_dict(data: Mapping[str, Any]) -> ListGroup:
    payload = dict(data)
    return ListGroup(
        type="list_group",
        id=str(payload.get("id", "")),
        ...
        items=[
            _list_group_item_from_dict(item)
...
def _list_group_item_from_dict(data: Mapping[str, Any]) -> ListGroupItem:
    payload = dict(data)
...
def _table_ref_from_dict(data: Mapping[str, Any]) -> TableRef:
    payload = dict(data)
    return TableRef(
        table_id=str(payload.get("table_id", "")),
        page=int(payload.get("page", 1)),
        type="table_ref",
        bbox=_bbox_from_serialized_value(payload.get("bbox")),
...
def _figure_ref_from_dict(data: Mapping[str, Any]) -> FigureRef:
    payload = dict(data)
    return FigureRef(
...
def _note_ref_from_dict(data: Mapping[str, Any]) -> NoteRef:
    payload = dict(data)
    return NoteRef(
...
def _assembly_element_from_dict(data: Mapping[str, Any]) -> AssemblyElement:
    payload = dict(data)
...
def _block_relation_from_dict(data: Mapping[str, Any]) -> BlockRelation:
    payload = dict(data)
...
def _page_stats_from_dict(data: Mapping[str, Any]) -> PageStats:
    payload = dict(data)
    return PageStats(
        page=int(payload.get("page", 1)),
...
def _assembly_warning_from_dict(data: Mapping[str, Any]) -> AssemblyWarning:
    payload = dict(data)
...
def _meta_from_dict(data: Mapping[str, Any]) -> AssemblyMeta:
    payload = dict(data)
    return AssemblyMeta(
...
def _bbox_from_serialized_value(value: Any) -> tuple[float, float, float, float] | None:
